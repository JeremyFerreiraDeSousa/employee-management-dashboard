...
import InputErrorMsg from '@/components/authentication/InputErrorMsg'
      ...
      <div className="group flex items-center bg-white h-12 overflow-hidden relative">
        {children && (
          <label
            ...
            className={`absolute top-1/2 -translate-y-1/2 left-4 text-gray-400 group-hover:text-blue-500 group-focus-within:text-blue-500 transition-colors duration-300${error ? ' text-red-500' : value ? ' text-green-500' : ''}`}
          >
            {children}
          </label>
        )}
